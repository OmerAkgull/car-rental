...
const CarFilter = () => {
  ...
  const types = ["All", "Mini", "Economic", "Compact", "Sedan", "Wagon", "Minivan", "SUV"];
      ...
      <Container>
          ...
          <Col md={9}>
            <Row>
              <div className="d-flex flex-wrap gap-3">
                {types.map((type) => (<FilterButton onclick={()=>setActiveFilter(type)} isActive={type === activeFilter} key={type} text={type}/>))}
              </div>
            </Row>
          </Col>
          <Col></Col>
        ...
        <Row className="mt-5">
          {filteredCars.map((car: Cars) => (
            <Col className="mt-5" key={car.id} md={4} sm={6}>
